...
resource "aws_network_interface_sg_attachment" "spoke1_attachment" {
  depends_on           = [aws_network_interface.eth0_spoke1]
  security_group_id    = aws_default_security_group.spoke_1.id
  network_interface_id = aws_network_interface.eth0_spoke1.id
}
